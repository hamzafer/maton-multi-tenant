...
interface Props {
  app: AppConfig;
  status: string | null; // null = not connected, "ACTIVE", "PENDING"
  onConnect: () => void;
  onDisconnect: () => void;
  onSelect: () => void;
  selected: boolean;
...
export default function AppCard({ app, status, onConnect, onDisconnect, onSelect, selected, loading }: Props) {
  ...
  return (
    <div
      ...
      className={`relative group border rounded-xl p-4 transition-all duration-300 ${
        selected
          ...
          : isConnected
          ? "border-border-default bg-surface-raised hover:border-accent/20 cursor-pointer"
    ...
    >
      ...
      <h3 className="text-[14px] font-semibold text-text-primary mb-0.5">{app.name}</h3>
      ...
      {!isConnected && !isPending && (
        <button
          ...
          disabled={loading}
          className="w-full text-[12px] font-medium py-1.5 rounded-lg border border-border-default text-text-secondary hover:text-text-primary hover:border-border-default hover:bg-surface-hover transition-all disabled:opacity-50"
        >
